...
class n2v_walk():
        ...
        while len(walk) < self.walk_length:
            ...
            if len(cur_nbrs) > 0:
                if len(walk) == 1:
                    ...
                    prev = walk[-2]
                    next = cur_nbrs[self.alias_draw(self.alias_edges[(prev, cur)][0],
    ...
    def preprocess_transition_probs(self):
        ...
        alias_nodes = {}
        for node in G.nodes():
            norm_const = len(list(G.neighbors(node)))
            normalized_probs = np.ones(norm_const) / norm_const
