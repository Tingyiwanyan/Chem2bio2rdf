    ...
    def train(self):
        ...
        for j in range(epoch):
            for i in range(iteration):
                batch_compound,batch_gene = self.get_one_batch(self.meta_path1,'c',i*self.batch_size)
                err_ = self.sess.run([self.negative_sum,self.train_step_neg],feed_dict={self.compound:batch_compound,
                                                                                        self.gene:batch_gene})
                print(f'epoch {j}\t error {err_[0]}')
